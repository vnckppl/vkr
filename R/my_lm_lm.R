...
my_lm_lm <-
    function(df, outcome, predictor, covariates = NULL, verbose = FALSE) {
  ...
  if (pred_class == "factor") {
    ...
    for (gr in seq(2, length(grvars))) {
      ...
      odf_vars <- c(
        odf_vars,
        paste(groups[gr], c("beta", "std", "p", "fdr", "eta2"), sep = "_"))
    }
    ...
    pr_std <- formatC(
      round(my_coeff[predictor, "Std. Error"], 2), format = "f", digits = 2)
    pr_pvl <- formatC(
    ...
    ## **** Combine this information
    odf <- rbind(pr_est, pr_std, pr_pvl, pr_fdr, pr_eta)

    ## **** Variable labels
    odf_vars <- c("beta", "std", "p", "fdr", "eta2")
